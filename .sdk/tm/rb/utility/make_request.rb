...
module ExtinctAnimalsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ExtinctAnimalsResponse.new({})
    ...
    if fetch_err
      ...
      response = ExtinctAnimalsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ExtinctAnimalsResponse.new(fetched)
    else
