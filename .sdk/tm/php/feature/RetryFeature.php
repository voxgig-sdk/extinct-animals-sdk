...
// ExtinctAnimals SDK retry feature
...
class ExtinctAnimalsRetryFeature extends ExtinctAnimalsBaseFeature
{
    ...
    public function init(ExtinctAnimalsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ExtinctAnimalsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ExtinctAnimalsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ExtinctAnimalsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
