...
[MCP](https://modelcontextprotocol.io) server exposing the ExtinctAnimals SDK as
two agent tools — `extinct-animals_list` and `extinct-animals_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/extinct-animals-mcp)
make build

# 2. Provide credentials via the environment
export EXTINCT_ANIMALS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user extinct-animals \
  -- /absolute/path/to/extinct-animals-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./extinct-animals-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// extinct-animals_list: first page of records
{ "entity": "animal" }
{ "entity": "animal", "query": { } }

// extinct-animals_load: one record by id
{ "entity": "animal", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/extinct-animals-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user extinct-animals \
     -- "$PWD"/dist/*/extinct-animals-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `extinct-animals_list` and `extinct-animals_load` tools now appear
   in new sessions. Ask the agent to *"list animal using extinct-animals"*
   and it calls `extinct-animals_list` with `{"entity":"animal"}`.
...
```sh
export EXTINCT_ANIMALS_APIKEY=sk_live_xxx            # API key
export EXTINCT_ANIMALS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `extinct-animals_list` tool
...
### Call the `extinct-animals_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `extinct-animals_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `extinct-animals_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EXTINCT_ANIMALS_APIKEY` | API key sent with every request. |
| `EXTINCT_ANIMALS_BASE` | Optional override of the API base URL. |
...
```sh
./extinct-animals-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"extinct-animals_load","arguments":{"entity":"animal","query":{"id":1}}}}'
```
