...
from extinctanimals_sdk.extinctanimals_types import (
    Animal,
    AnimalLoadMatch,
    AnimalListMatch,
)
...
class AnimalEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AnimalEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Animal:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Animal().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Animal]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Animal().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
