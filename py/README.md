...
The Python SDK for the ExtinctAnimals API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Animal(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Animal(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ExtinctAnimals(None).load(
    {"id": "test01"}, None
)
