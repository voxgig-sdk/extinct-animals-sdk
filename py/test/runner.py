# ExtinctAnimals SDK test runner
...
class ExtinctAnimalsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ExtinctAnimalsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ExtinctAnimalsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ExtinctAnimalsTestRunner.getenv("EXTINCT_ANIMALS_TEST_LIVE")
        override = ExtinctAnimalsTestRunner.getenv("EXTINCT_ANIMALS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ExtinctAnimalsTestRunner.getenv(key)
        ...
        explain = ExtinctAnimalsTestRunner.getenv("EXTINCT_ANIMALS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ExtinctAnimalsTestRunner._test_control is not None:
            return ExtinctAnimalsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ExtinctAnimalsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ExtinctAnimalsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ExtinctAnimalsTestRunner.load_test_control()
...
def load_env_local():
    ExtinctAnimalsTestRunner.load_env_local()


def env_override(m):
    return ExtinctAnimalsTestRunner.env_override(m)


def entity_data(v):
    return ExtinctAnimalsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ExtinctAnimalsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ExtinctAnimalsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ExtinctAnimalsTestRunner.load_test_control()


def live_delay_ms():
    return ExtinctAnimalsTestRunner.live_delay_ms()
