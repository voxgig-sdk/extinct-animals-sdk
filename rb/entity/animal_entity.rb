...
class AnimalEntity
  ...
  def make
    opts = @_entopts.dup
    AnimalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ExtinctAnimalsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Animal, Hash] the current Animal data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ExtinctAnimalsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Animal fields)
  def match_get
  ...
  #
  # @param reqmatch [AnimalLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Animal.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Animal, Hash] the loaded Animal; raises ExtinctAnimalsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ExtinctAnimalsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [AnimalListMatch, Hash, nil] match filter (any subset of
  #   Animal fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Animal>, Array] the matching Animal items; raises ExtinctAnimalsError on failure
