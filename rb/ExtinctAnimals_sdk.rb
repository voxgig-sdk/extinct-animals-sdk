...
require_relative 'ExtinctAnimals_types'
...
class ExtinctAnimalsSDK
  ...
  def initialize(options = {})
    ...
    utility = ExtinctAnimalsUtility.new
    @_utility = utility
    ...
    feature_opts = ExtinctAnimalsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ExtinctAnimalsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ExtinctAnimalsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ExtinctAnimalsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ExtinctAnimalsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ExtinctAnimalsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ExtinctAnimalsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = ExtinctAnimalsSpec.new({
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ExtinctAnimalsError => err
    ...
    if fetched.is_a?(Hash)
      status = ExtinctAnimalsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ExtinctAnimalsError.new(
        "graphql_error", "ExtinctAnimalsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ExtinctAnimalsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
