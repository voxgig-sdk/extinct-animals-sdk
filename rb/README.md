...
```ruby
require_relative "ExtinctAnimals_sdk"
client = ExtinctAnimalsSDK.new(options)
```
...
raise a `ExtinctAnimalsError` (a `StandardError` subclass), so wrap
...
```
rb/
├── ExtinctAnimals_sdk.rb       -- Main SDK module
...
The main module (`ExtinctAnimals_sdk`) exports the SDK class
