...
//     (\`new ExtinctAnimalsSDK(...)\` / \`ExtinctAnimalsSDK.test(...)\` become
//     \`ExtinctAnimalsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ExtinctAnimalsSDK'
...
const TEST_SEED = {"entity":{"animal":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ExtinctAnimalsSDK(...)\` and any
// \`ExtinctAnimalsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ExtinctAnimalsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ExtinctAnimalsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
