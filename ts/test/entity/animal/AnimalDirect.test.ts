...
describe('AnimalDirect', async () => {
  ...
  test('direct-load-animal', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-animal', setup.live)) return
  ...
  test('direct-list-animal', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-animal', setup.live)) return
    const { client, calls } = setup
