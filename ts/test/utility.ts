...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.EXTINCTANIMALS_TEST_LIVE ||
    'TRUE' === process.env.EXTINCTANIMALS_TEST_OVERRIDE
  ) {
  ...
  m.EXTINCTANIMALS_TEST_EXPLAIN = process.env.EXTINCTANIMALS_TEST_EXPLAIN || m.EXTINCTANIMALS_TEST_EXPLAIN
