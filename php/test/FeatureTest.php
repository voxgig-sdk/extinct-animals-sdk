...
// ExtinctAnimals SDK feature test
//
...
class FtCtrl extends ExtinctAnimalsControl
{
...
class FtClient
{
    ...
    private ExtinctAnimalsUtility $utility;

    public function __construct(ExtinctAnimalsUtility $utility, array $options)
    {
    ...
    public function get_utility(): ExtinctAnimalsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ExtinctAnimalsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ExtinctAnimalsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ExtinctAnimalsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ExtinctAnimalsConfig::make_feature($name);
    ...
    private static function buildUrl(ExtinctAnimalsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ExtinctAnimalsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ExtinctAnimalsResponse($fetched);
        }
    ...
    private function fail(ExtinctAnimalsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ExtinctAnimalsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
