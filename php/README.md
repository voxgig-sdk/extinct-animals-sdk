...
The PHP SDK for the ExtinctAnimals API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Animal(null)->list(null, null);
...
```php
[$result, $err] = $client->Animal(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->ExtinctAnimals(null)->load(
    ["id" => "test01"], null
);
