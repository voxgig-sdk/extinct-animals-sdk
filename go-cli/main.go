// Command ExtinctAnimals-cli is a CLI + REPL for the ExtinctAnimals SDK,
...
const prompt = "extinct-animals"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "animal"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EXTINCT_ANIMALS_APIKEY (api key), EXTINCT_ANIMALS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EXTINCT_ANIMALS_APIKEY carries the API key and
	// EXTINCT_ANIMALS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EXTINCT_ANIMALS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EXTINCT_ANIMALS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewExtinctAnimalsSDK(opts)
