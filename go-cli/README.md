...
boru-driven command-line client **and** interactive REPL for the ExtinctAnimals
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/extinct-animals-cli)
make build

# 2. See usage (words, entities, env vars)
./extinct-animals-cli --help

# 3. Provide credentials once, via the environment
export EXTINCT_ANIMALS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./extinct-animals-cli list animal
./extinct-animals-cli load 1 animal            # {id:1} shorthand
./extinct-animals-cli load '{id:1}' animal       # explicit match map

# 5. Override the API base URL for a single call
EXTINCT_ANIMALS_BASE=https://api.example.com ./extinct-animals-cli list animal

# 6. No arguments -> interactive REPL
./extinct-animals-cli
extinct-animals> list animal
extinct-animals> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/extinct-animals-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/extinct-animals-cli list animal
   ```
...
```sh
./extinct-animals-cli load 1 animal          # scalar shorthand for {id:1}
./extinct-animals-cli load '{id:1}' animal     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `extinct-animals>`). Each line is
evaluated as its own boru expression:

```text
$ ./extinct-animals-cli
extinct-animals> list animal
extinct-animals> /help
extinct-animals> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `animal`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/extinct-animals-cli`. |
...
(`./extinct-animals-cli <expr>`) and interactively (the REPL), and expressions compose the
