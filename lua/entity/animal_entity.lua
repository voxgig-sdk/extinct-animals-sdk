...
local AnimalEntity = {}
AnimalEntity.__index = AnimalEntity


function AnimalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AnimalEntity)
  self._name = "animal"
  self._client = client
...
function AnimalEntity:get_name()
  return self._name
end


function AnimalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AnimalEntity.new(self._client, opts)
end
...
function AnimalEntity:mark_deleted()
  self._deleted = true
end


function AnimalEntity:deleted()
  return true == self._deleted
end


function AnimalEntity:data_set(args)
  if args ~= nil then
...
function AnimalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AnimalEntity:match_set(args)
  if args ~= nil then
...
function AnimalEntity:match_get()
...
function AnimalEntity:stream(action, args, callopts)
...
function AnimalEntity:_run_op(ctx, post_done)
  local utility = self._utility
