...
describe("AnimalDirect", function()
  it("should direct-list-animal", function()
    local setup = animal_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-animal", setup.live and "live" or "unit")
  ...
  it("should direct-load-animal", function()
    local setup = animal_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-animal", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function animal_direct_setup(mockres)
